...
const handleGuestAddToCart = (product: Product, quantity: number, selectedVariant?: ProductVariant): Cart => {
  ...
  const existingItemIndex = guestCart.items.findIndex(
    item => item && item.product && item.product._id === product._id && 
            (item.variantId === selectedVariant?._id || item.selectedVariant?._id === selectedVariant?._id)
  );
  
  if (existingItemIndex > -1) {
    guestCart.items[existingItemIndex].quantity += quantity;
    guestCart.items[existingItemIndex].price = selectedVariant?.price || product.basePrice;
    ...
    // ✅ CRITICAL: Create item with ALL fields including variantId and variantName
    const newItem: CartItem = {
      _id: guestItemId,
      product,
      selectedVariant, // Keep for backward compatibility
      variantId: selectedVariant?._id,        // ✅ MUST HAVE THIS
      variantName: selectedVariant?.variantName, // ✅ MUST HAVE THIS
      quantity,
    ...
    console.log('🛒 Created new item with:', {
  ...
  console.log('🛒 Saved guest cart with items:', guestCart.items.map(item => ({
    id: item._id,
...
const addToCart = async (product: Product, quantity: number, selectedVariant?: ProductVariant) => {
  ...
  // ✅ CRITICAL FIX: If no variant selected but product has variants, use the first one
  if (!selectedVariant && product.variants && product.variants.length > 0) {
    selectedVariant = product.variants[0];
    console.log('🛒 AUTO-SELECTED first variant:', selectedVariant.variantName);
  }
  ...
  try {
    ...
    if (isGuest) {
      ...
      const updatedCart = await cartAPI.addToCart({
        ...
        variantId: selectedVariant?._id || selectedVariant?.variantName,
        quantity
      });
      setCart(updatedCart);
    }
