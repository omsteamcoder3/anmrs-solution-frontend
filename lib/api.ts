...
getActive: async (): Promise<{ success: boolean; data: Category[]; count: number }> => {
  const res = await fetchAPI<{ success: boolean; data: Category[]; count: number }>('/categories');

  return {
    ...res,
    data: res.data.filter((c) => c.isActive !== false),
    count: res.data.filter((c) => c.isActive !== false).length
  };
}
};
